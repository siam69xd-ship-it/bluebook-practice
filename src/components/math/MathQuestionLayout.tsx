    ...
    <div className={cn(
      ...
      <header className="sticky top-0 z-30 bg-white border-b border-gray-200">
        <div className="flex items-center justify-between px-4 h-14">
          <div className="flex items-center gap-3">
            <button onClick={() => navigate('/practice')} className="p-2 rounded-md hover:bg-gray-100 transition-colors">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => setIsTimerHidden(!isTimerHidden)} className="flex items-center gap-1.5 px-3 py-1.5 text-sm text-gray-700 hover:bg-gray-100 rounded-md transition-colors border border-gray-300">
              <span className="flex gap-0.5"><span className="w-0.5 h-3 bg-gray-700"></span><span className="w-0.5 h-3 bg-gray-700"></span></span>
              {isTimerHidden ? 'Show' : 'Hide'}
            </button>
          </div>
          <div className="flex items-center gap-1">
            <button onClick={() => setShowCalculator(!showCalculator)} className={cn("flex flex-col items-center gap-0.5 px-3 py-1 rounded-md transition-colors", showCalculator ? "bg-gray-100" : "hover:bg-gray-100")}>
              <Calculator className="w-5 h-5 text-gray-700" />
              <span className="text-xs text-gray-600">Calculator</span>
            </button>
            <button onClick={() => setShowReference(!showReference)} className={cn("flex flex-col items-center gap-0.5 px-3 py-1 rounded-md transition-colors", showReference ? "bg-gray-100" : "hover:bg-gray-100")}>
              <BookOpen className="w-5 h-5 text-gray-700" />
              <span className="text-xs text-gray-600">Reference</span>
            </button>
            <button onClick={toggleFullscreen} className="flex flex-col items-center gap-0.5 px-3 py-1 rounded-md hover:bg-gray-100 transition-colors">
            ...
            <motion.div key={currentQuestion.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.2 }}>
              <div className="flex items-center justify-between mb-6 pb-4 border-b border-gray-100">
                <div className="flex items-center gap-4">
                  <span className="flex items-center justify-center w-10 h-10 bg-gray-900 text-white text-base font-bold rounded">{currentIndex + 1}</span>
                  <button onClick={handleToggleMark} className={cn("flex items-center gap-2 px-3 py-2 rounded-md text-sm transition-colors", currentState?.markedForReview ? "bg-amber-50 text-amber-700" : "text-gray-600 hover:bg-gray-100")}>
                    <Bookmark className={cn("w-4 h-4", currentState?.markedForReview && "fill-amber-500")} /> Mark for Review
                  </button>
                </div>
                <div className="flex items-center gap-3">
                  <button className="flex items-center gap-1 text-sm text-gray-500 hover:text-gray-700"><Flag className="w-4 h-4" /> Report</button>
                  <button onClick={() => setIsEliminationMode(!isEliminationMode)} className={cn("flex items-center justify-center w-8 h-8 rounded transition-colors", isEliminationMode ? "bg-gray-900 text-white" : "text-gray-500 hover:bg-gray-100 border border-gray-300")} title={isEliminationMode ? "Exit elimination mode" : "Enter elimination mode"}>
                    <span className="relative text-sm font-bold">S<span className="absolute inset-0 flex items-center justify-center"><span className="w-full h-[1.5px] bg-current rotate-[-20deg]" /></span></span>
                  </button>
                  {hasEliminations && <button onClick={handleUndoEliminations} className="flex items-center gap-1 text-sm text-gray-500 hover:text-gray-700" title="Undo all eliminations"><Undo2 className="w-4 h-4" /> Undo</button>}
                </div>
              </div>

              <div className="mb-8">
                <LatexRenderer content={currentQuestion.questionPrompt || ''} className="text-[17px] leading-relaxed text-gray-900" />
      ...
      <footer className="sticky bottom-0 bg-white border-t border-gray-200">
        <div className="flex items-center justify-between px-4 py-3">
          <button onClick={() => setShowNavigator(true)} className="flex items-center gap-2 px-4 py-2 bg-gray-900 text-white rounded-full text-sm font-medium hover:bg-gray-800 transition-colors">
            {currentIndex + 1} of {questions.length} <ChevronDown className="w-4 h-4" />
          </button>
          <div className="flex items-center gap-2">
            <button className="p-2 rounded-full hover:bg-gray-100 transition-colors"><Info className="w-5 h-5 text-gray-500" /></button>
            ...
            {!(currentState?.checked && isCorrect) ? (
              <button
                ...
                className={cn("flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium transition-colors", currentState?.userAnswer ? "bg-[#e6f7ff] text-[#0077cc] hover:bg-[#cceeff] border border-[#0077cc]" : "text-gray-400 border border-gray-200 cursor-not-allowed")}
              >
          ...
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" onClick={() => onNavigate('prev')} disabled={currentIndex === 0} className="px-5 border-gray-300 text-gray-700">Previous</Button>
            <Button variant="outline" size="sm" onClick={() => onNavigate('next')} disabled={currentIndex === questions.length - 1} className="px-5 border-gray-300 text-gray-700">Next</Button>
          </div>
        </div>
      </footer>
